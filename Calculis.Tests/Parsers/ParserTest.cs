...
namespace Calculis.Tests.Parsers
{
    public class ParserTest
    {
        private ExpressionParser _parser = new(new ItemsManager(new List<IItem>()));
        ...
        [InlineData("item1>item2*item3", "MUL(GREATER(item1;item2);item3)")]
        [InlineData("item1<=item2*item3", "MUL(LESSOREQUAL(item1;item2);item3)")]
        [InlineData("item1<=(item2*item3)", "LESSOREQUAL(item1;MUL(item2;item3))")]
